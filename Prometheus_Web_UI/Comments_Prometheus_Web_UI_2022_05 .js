{/*  ====  WWW /БРАУЗЕР / АДРЕССНАЯ СТРОКА  ====

    +++ WWW(world wide web) - всемирная паутина дает широкие возможности, от просмотра прогноза погоды и заказа пиццы, до 
            проведения сложных финансовых операций или даже создания всемирной революции. К вебу можно получить доступ с
            телефона, компьютера, смарт-часов. www и интернет это разные вещи.
        

        Интернет - возможность компьютеров соединяться друг с другом и обмениваться файлами. Когда интернет только появился
            компьютеры могли соединяться между собой только на прямую. Когда появился веб коммуникация в плане доступа к
            интернету значительно упростилась и появились новый возможности, такие как обмен имейлами/файлами. Интернет
            и предоставляет доступ к всемироной паутине(веб).

        Веб - це впорядковане віртуальне середовище, де люди можуть шукати інформацію, додавати і обмінюватись нею,
            можно сравнить с городом, застроенным домами. Каждый дом - веб сервер. Если кто то хочет сделать сайт он
            должен орендовать квартиру в таком доме(место на сервере) что бы другие пользователи тоже имели доступ к
            сайту. Хозяин дома(сервера) получает орендную плату за то что мы розмещаем там сайт. Сервер называется
            - web host. Серверов много и на них хостится миллионы сайтов, чтобы найти нужный сайт пользователь должен
            ввести веб адресс (google.com) - он указывает на место сайта на сервере. Информация на странице базируется
            на языках HTML и JavaScript, но пользователь этого не должен знать и поэтому браузер берет на себя ф-ю
            превращения написанного кода в страницу. Благодаря гиперссылкам можно переходить на страницы в рамках сайта
            или на сайты не вводя каждый раз адресс в адрессную строку браузера.



    +++ Браузер, как устроен.

        Вся информация в веб представлена кодом, а браузер работает переводчиком этого кода. Есть разные браузеры и мы должны
            их знать чтобы наш сайт правильно отображался на каждом. Основные: Google - Chrome, mozilla - Firefox, Microsoft 
            - IE и Edge, Opera, Apple - Safari. Браузеры по разному обрабатывают информацию. Для проверки поддержки браузером
            тех или иных возможностей(кода) существуют такие ресурсы - https://caniuse.com и https://html5please.com . Когда
            новые возможности языков разметки только добавляются и тестируются они становятся доступны в браузере через 
            вендорные префиксы.

        Структура Браузера: User interface - то что видит пользователь когда открывает браузер(поле ввода, кнопки, закладки,
            окно страницы), Browser engine - это мостик между User interface и рендерной машиной, Render engine(рендерная 
            машина) - отвечает за то чтобы прочитать, проанализировать и отобразить ХТМЛ страницу. В Render engine заходят
            три блока: Networking - часть которая отвечает за связь браузера с сервером, JavaScript Interpreter - виртуальная
            машина которая читает, анилизирует и выполняет JS код, UI Backend (user interface backend) - реализация стандартных
            виджетов(чекбоксы, радиобаттоны, ...). Еще одна часть браузера - Data Storage - место на локальной машине где
            браузер сохраняет некоторые данные(логины, пароли,...).
            


    +++ Что происходит когда пользователь вводит адресс в адрессную строку(коммуникация браузер и сервера)

        Сначала браузер анализирует этот адрес(url - уницированный локатор ресурсов). URL - стандартная форма записи адресса
            ресурса. URL формируется так: 1) протокол(определенная схема обращения к ресурсу) - http, https, ftp. 2) хост - 
            полное доменное(на каком сервере зарегистрирован сайт www.facebook.com) имя или IP адресс ресурса. 3) параметры -
             url адресса(/svitlana.sikora=friends) - они говорят какую именно страницу мы ищем или
            какие данные хотим получить от сервера. 4) Якорь - не обязательная часть. Он предназначен для внутренней адрессации
            по ресурсу.

        После анализа url браузер выделяет из него доменное имя(wikipedia.org) - это идентификатор который присвоен 
            определенному сетевому ресурсу. Оно записывается в виде строки, а не айпи адресса чтобы пользователь мог его 
            запомнить и проще им пользоваться. IP адресс - уникальный числовой номер, предназначенный для адрессации компов и
            приспособлений в сети.

        Дальше браузер обращается к DNS(система доменный имен) для того чтобы превратить строку доменного имени на
            соответствующий ей IP адресс. ДНС можно сравнить в большим справочником где прописаны все названия доменов и их
            IP адресса. ДНС серверов существует несколько, если на одном не находится нужное доменное имя, тогда браузер идет
            на другой, и тд. пока не найдет доменное имя и тогда с ДНС в браузер отправляется ответ в виде IP адресса. После
            получения ответа от ДНС сервера браузер делает запрос на web сервер, для этому ему необходимо использовать
            протокол, в основном это http(hyper text transport protokol), хотя есть и другие - сетевой протокол который 
            использует браузер для получения веб страницы с веб сервера. При первом запросе сервер отправляет в ответ ХТМЛ 
            страницу. Браузер получает эту страницу, анализирует, раскладывает на составляющие части и отображает, если по
            мере анализа браузер находит линки(ссылки) на другие ресурсы такие как картинки, видео, документы он делает запрос
            на каждый сервер отдельно по каждому такому линку, а в результате пользователь видит полностью готовую отображаемую
            страницу.
